...
inline _CharT*
__VALUE_TYPE(const  _DBG_iter_base< __STL_DBG_STRING_BASE >&) {
return (_CharT*)0;
}
template <class _CharT, class _Traits, class _Alloc>
inline random_access_iterator_tag
__ITERATOR_CATEGORY(const  _DBG_iter_base< __STL_DBG_STRING_BASE >&) {
...
public:
  ...
  __STL_USING_BASE_MEMBER __STL_DBG_STRING_BASE::size;
...
public:                         // Substring.
  _Self substr(size_type __pos = 0, size_type __n = _Base::npos) const {
    ...
    return _Self(this->begin() + __pos, 
                 this->begin() + __pos + min(__n, size() - __pos));
  }
